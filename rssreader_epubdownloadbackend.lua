...
local function getUrlContent(url, cookies, timeout, maxtime, add_to_cache)
    ...
    if numeric_code and numeric_code >= 400 and numeric_code < 500 then
...
function EpubDownloadBackend:createEpub(epub_path, html, url, include_images, message, filter_enable, filter_element)
    ...
    local balanced_html = cre.getBalancedHTML(html, 0x0)
    if type(balanced_html) == "string" and balanced_html ~= "" then
        html = balanced_html
    else
        logger.warn("EpubDownloadBackend", "Balanced HTML unavailable; continuing with original markup")
    end
